...
function ActionDetail({ item, close }: { item: Action; close: () => void }) {
  ...
  return <section className="inspection-detail" aria-label="Action details"><div className="modal-heading"><div><h2 id="dialog-title">{item.tool_name}</h2><p>{item.title}</p></div><button className="icon-button" aria-label="Close action details" onClick={close}><X size={20} /></button></div><div className="safety-detail">
...
export function SafetyWorkspace({ connections, refresh, notify, notifications }: { notifications: SafetyNotifications; connections: Connection[]; refresh: () => void; notify: (message: string) => void }) {
  ...
  const status = useQuery({ queryKey: ["safety"], queryFn: () => api<{ model: string; key_configured: boolean; key_file: string; workers: unknown[]; counts: Record<string, number>; performance?: { requests: number; automatic_pause: { p95_ms: number | null; samples: number }; failed: number; expired: number; missing_receipts: number; truncated: boolean } }>("/safety") });
  ...
  const params = new URLSearchParams({ connection, tool, start: range.start, end: range.end });
  ...
  useEffect(() => { setOffset(0); setOpened(null); }, [outcome, tool, historyScope]);
  useEffect(() => {
    if (!opened) return;
